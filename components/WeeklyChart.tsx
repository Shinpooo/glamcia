...
const TimeChart: React.FC = () => {
  ...
  const { timeData, chartData } = useMemo(() => {
    ...
    switch (timeUnit) {
      case 'day':
        startDate.setDate(endDate.getDate() - 13); // 14 jours
        intervals = eachDayOfInterval({ start: startDate, end: endDate });
        break;
      case 'week':
        startDate.setDate(endDate.getDate() - (8 * 7)); // 8 semaines
        ...
        break;
      case 'month':
        startDate.setMonth(endDate.getMonth() - 11); // 12 mois
      ...
      tooltip: {
        callbacks: {
          title: (context) => {
            ...
            const unitLabel = timeUnit === 'day' ? 'Jour du' : timeUnit === 'week' ? 'Semaine du' : 'Mois de';
  ...
  const navigatePrevious = () => {
    switch (timeUnit) {
      case 'day':
        setCurrentPeriod(prev => subDays(prev, 14));
        break;
      case 'week':
        setCurrentPeriod(prev => subWeeks(prev, 8));
        break;
      case 'month':
        setCurrentPeriod(prev => subMonths(prev, 12));
        break;
      case 'year':
  ...
  const navigateNext = () => {
    switch (timeUnit) {
      case 'day':
        setCurrentPeriod(prev => addDays(prev, 14));
        break;
      case 'week':
        setCurrentPeriod(prev => addWeeks(prev, 8));
        break;
      case 'month':
        setCurrentPeriod(prev => addMonths(prev, 12));
        break;
      case 'year':
  ...
  const canNavigateNext = () => {
    ...
    switch (timeUnit) {
      case 'day':
        ...
        break;
      case 'week':
        nextPeriod.setDate(nextPeriod.getDate() + (8 * 7));
        break;
      case 'month':
        nextPeriod.setMonth(nextPeriod.getMonth() + 12);
        break;
      case 'year':
      ...
      <div className="flex flex-col gap-4 mb-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-600">Revenus vs Dépenses par catégorie ({currentConfig.period})</p>
          </div>
        ...
        <div className="flex items-center justify-between bg-gray-50 rounded-xl p-4">
          <button
            ...
            className="flex items-center space-x-2 px-4 py-2 bg-white rounded-lg shadow-sm hover:shadow-md transition-all hover:bg-gray-50"
          >
          ...
          <div className="flex items-center space-x-4">
            <div className="text-center">
              <p className="text-sm font-semibold text-gray-900">{getCurrentPeriodDescription()}</p>
              <p className="text-xs text-gray-500">{currentConfig.period}</p>
            </div>
            
            <button
              onClick={resetToToday}
              className="flex items-center space-x-1 px-3 py-1 bg-pink-100 text-pink-700 rounded-lg hover:bg-pink-200 transition-colors text-sm"
      ...
      <div className="mb-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-green-50 rounded-xl p-4">
          <div className="flex items-center space-x-2 mb-3">
            ...
            <h4 className="text-sm font-semibold text-green-700">Prestations (Revenus)</h4>
          </div>
        ...
        <div className="bg-red-50 rounded-xl p-4">
          <div className="flex items-center space-x-2 mb-3">
            ...
            <h4 className="text-sm font-semibold text-red-700">Dépenses</h4>
          </div>
          <div className="grid grid-cols-2 gap-2 text-xs">
        ...
        <div className="bg-emerald-50 rounded-xl p-4">
          <div className="flex items-center space-x-2 mb-3">
            ...
            <h4 className="text-sm font-semibold text-emerald-700">Évolution du Bénéfice</h4>
          </div>
          <div className="space-y-2 text-xs">
            <div className="flex items-center space-x-2">
              <div className="w-8 h-0.5 bg-emerald-600 rounded"></div>
              <span className="text-gray-700">Ligne de tendance</span>
            </div>
            <div className="flex items-center space-x-2">
              <div className="w-3 h-3 bg-green-500 rounded-full"></div>
              <span className="text-gray-700">Bénéfice positif</span>
            </div>
            <div className="flex items-center space-x-2">
              <div className="w-3 h-3 bg-red-500 rounded-full"></div>
              <span className="text-gray-700">Bénéfice négatif</span>
            </div>
          </div>
        </div>
      </div>
